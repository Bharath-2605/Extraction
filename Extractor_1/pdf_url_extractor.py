...
def _chunk_holds_print(array) -> bool:
    """A flat black-and-white chunk can hold a printed link. Photos do not."""
    if array is None or getattr(array, "size", 0) == 0 or min(array.shape[:2]) < 8:
        return False
    step = max(1, int(max(array.shape[:2]) / 72))
    small = array[::step, ::step]
    gray = small if getattr(small, "ndim", 0) == 2 else small.min(axis=2)
    dark = float((gray < 90).mean())
    if dark < 0.012 or dark > 0.7:
        return False
    mid = float(((gray >= 90) & (gray < 210)).mean())
    return mid < 0.42


def _cluster_chunk_rects(rects: list[fitz.Rect], page_rect: fitz.Rect) -> list[fitz.Rect]:
    """Join neighboring image pieces that together make one code or one link."""
    pending = [
        rect
        for rect in rects
        if rect is not None
        and min(float(rect.width), float(rect.height)) >= 4
        and max(float(rect.width), float(rect.height)) <= 180
    ]
    ...
    for left_index, left in enumerate(pending):
        grown = fitz.Rect(left.x0 - gap, left.y0 - gap, left.x1 + gap, left.y1 + gap)
        for right_index in range(left_index + 1, len(pending)):
            if grown.intersects(pending[right_index]):
                parent[find(right_index)] = find(left_index)
    groups: dict[int, list[fitz.Rect]] = {}
    for index, rect in enumerate(pending):
    ...
    for group in groups.values():
        ...
        for rect in group[1:]:
            union |= rect
        if min(float(union.width), float(union.height)) < 16:
            continue
        if float(union.width) * float(union.height) > 0.22 * page_area:
            continue
        unions.append(union)
    unions.sort(key=lambda rect: float(rect.width) * float(rect.height), reverse=True)
    return unions[:3]
...
def extract_from_page(page: fitz.Page, document: fitz.Document) -> tuple[list[dict], str]:
    ...
    if banner is not None:
        ...
        ocr_seen_boxes.append(banner)

    def _recover_skipped_chunks() -> None:
        """Read image chunks the pass above did not reach, without scanning the page again."""
        from vision_scan import _fit_image, _qr_points_present
        ...
        skipped_rects = [
            ...
            and not _rect_covered(image.get("bbox"), ocr_seen_boxes, overlap=0.8)
            and not _rect_covered(image.get("bbox"), covered, overlap=0.8)
            and not (banner is not None and _same_qr_spot(image.get("bbox"), banner))
        ]

        def _already_ocr(rect: fitz.Rect | None) -> bool:
            return rect is not None and _rect_covered(rect, ocr_seen_boxes, overlap=0.8)

        def _preview(array):
            if array is None or max(array.shape[:2]) <= 700:
                return array
            return _fit_image(array, 700)

        def _try_qr(array, bbox, location: str) -> None:
            nonlocal extra_qr, qr_checks
            if qr_checks >= 12 or extra_qr >= 8 or array is None or _already_found(bbox):
                return
            if bbox is not None and _rect_covered(bbox, covered, overlap=0.8):
                return
            qr_checks += 1
            preview = _preview(array)
            if preview is None or _mostly_blank(preview):
                return
            try:
                present = _qr_points_present(preview)
            except Exception:
                present = False
            if not present:
                return
            before = len(placed)
            _decode_picture(preview, bbox, location)
            if len(placed) > before:
                extra_qr += 1

        def _try_ocr(array, bbox, location: str) -> None:
            nonlocal extra_ocr
            if extra_ocr >= 4 or array is None or _already_ocr(bbox):
                return
            preview = _preview(array)
            if preview is None or not _chunk_holds_print(preview) or _mostly_blank(preview):
                return
            ocr_jobs.append((location, _source_for_image(bbox, page_rect), pool.submit(recognize_text, preview, True)))
            if bbox is not None:
                ocr_seen_boxes.append(bbox)
            extra_ocr += 1
        ...
        for image in images:
            ...
            if bbox is None or array is None:
                continue
            if banner is not None and _same_qr_spot(bbox, banner):
                continue
            frac = _image_area_frac(bbox, page_rect)
            if frac > 0.55:
                continue
            side = min(float(bbox.width), float(bbox.height))
            aspect = float(bbox.width) / max(float(bbox.height), 1.0)
            square = 0 if 0.7 <= aspect <= 1.45 and side >= 16 else 1
            wide = 0 if aspect >= 2.0 and float(bbox.height) <= 80 else 1
            pending.append((square, wide, frac, image))
        pending.sort(key=lambda item: item[:3])
        for _square, _wide, frac, image in pending:
            bbox = image.get("bbox")
            location = location_for_bbox(tuple(bbox), page_height)
            array = image.get("array")
            _try_qr(array, bbox, location)
            # Square chunks are codes, already handled above. OCR is for a printed link.
            if frac <= 0.28 and not _is_qr_like(array, bbox):
                _try_ocr(array, bbox, location)

        for union in _cluster_chunk_rects(skipped_rects, page_rect):
            if _already_found(union) or _already_ocr(union):
                continue
            if any(_rect_covered(union, [image.get("bbox")], overlap=0.85) for image in images if image.get("bbox") is not None):
                continue
            try:
                array = render_region(page, union, _qr_zoom_for_rect(union, target=480))
            except Exception:
                continue
            location = location_for_bbox(tuple(union), page_height)
            _try_qr(array, union, location)
            _try_ocr(array, union, location)

    _recover_skipped_chunks()
