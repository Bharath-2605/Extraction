...
def _candidate_urls(raw: str) -> list[str]:
    ...
    if found:
        return [_drop_leading_caption(url) for url in found]
    cleaned = strip_trailing_punctuation(text)
    if cleaned.lower().startswith(("http://", "https://", "ftp://", "www.")):
        return [_drop_leading_caption(cleaned)]
    return []


def _drop_leading_caption(url: str) -> str:
    """securely.virginredrewards.syf.com is the caption glued onto the link."""
    text = str(url or "")
    prefix = ""
    for scheme in ("https://", "http://", "ftp://"):
        if text.lower().startswith(scheme):
            prefix = text[: len(scheme)]
            text = text[len(scheme) :]
            break
    host, sep, rest = text.partition("/")
    labels = [part for part in host.split(".") if part]
    if len(labels) < 3 or labels[0].lower() != "securely":
        return url
    host = ".".join(labels[1:])
    return prefix + host + (sep + rest if sep else "")
...
def extract_from_page(page: fitz.Page, document: fitz.Document) -> tuple[list[dict], str]:
    ...
    def _queue_chunk_text(image, hit_box, clip: fitz.Rect) -> None:
        """Read the words in the same chunk, to the left of the code."""
        if image is None or getattr(image, "size", 0) == 0:
            return
        text_image = image
        if hit_box is not None:
            x0, y0, x1, y1 = (int(hit_box[0]), int(hit_box[1]), int(hit_box[2]), int(hit_box[3]))
            span = max(x1 - x0, 8)
            x_start = max(0, x0 - int(span * 3.6))
            y_start = max(0, y0 - 8)
            y_end = min(int(image.shape[0]), max(y1 + 12, y_start + 8))
            if x0 - x_start > 8:
                text_image = image[y_start:y_end, x_start:x0]
        if text_image is None or getattr(text_image, "size", 0) == 0 or _mostly_blank(text_image):
            return
        ocr_jobs.append(
            (
                location_for_bbox(tuple(clip), page_height),
                "image",
                pool.submit(recognize_text, text_image, True),
            )
        )

    def _ocr_link_beside_qr(box: fitz.Rect | None, clip) -> None:
        """Read the link when it is printed beside a small code, not inside that picture."""
        if box is None or max(float(box.width), float(box.height)) > 90:
            return
        if clip is not None and float(clip.width) > float(box.width) * 1.8:
            return
        pad = min(240.0, max(float(box.width) * 4.5, 120.0))
        band = fitz.Rect(box.x0 - pad, box.y0 - 12, box.x0 - 1, box.y1 + 18) & page_rect
        if band.is_empty or float(band.width) < 24:
            return
        try:
            picture = render_region(page, band, _qr_zoom_for_rect(band, target=640))
        except Exception:
            return
        if picture is None or min(picture.shape[:2]) < 8 or _mostly_blank(picture):
            return
        ocr_jobs.append(
            (
                location_for_bbox(tuple(band), page_height),
                "image",
                pool.submit(recognize_text, picture, True),
            )
        )
    ...
    def record_hits(hits, location: str, clip, shape) -> None:
        for hit in hits or []:
            ...
            _ocr_link_beside_qr(page_box, clip)
    ...
    def _qr_image_rank(item: dict) -> tuple:
        ...
        # Small chunks first, then square codes, then larger pictures.
        if side <= 160:
            return (0, _image_area_frac(bbox, page_rect))
        return (1 if square else 2, -side if square else _image_area_frac(bbox, page_rect))
        ...
        for row in range(rows - 1, -1, -1):
            for col in range(cols - 1, -1, -1):
                ...
                if picture is None:
                    picture = preview
                before = len(placed)
                hits = _opencv_prepared_hits(picture)
                record_hits(
                    hits,
                    location_for_bbox(tuple(clip), page_height),
                    clip,
                ...
                if len(placed) > before:
                    box = placed[-1].get("box")
                    small = box is not None and max(float(box.width), float(box.height)) < 140
                    if small and hits:
                        _queue_chunk_text(picture, hits[0].get("box"), clip)
                    return
    ...
    for image in images:
        ...
        if hide_qr:
            ...
            continue
        # A link printed in a small chunk, logo, or caption beside a small code.
        small_chunk = bbox is not None and min(float(bbox.width), float(bbox.height)) <= 160 and frac <= 0.18
        if image.get("array") is None or frac > 0.18 or (logo_seen >= 6 and not small_chunk):
            if image.get("array") is not None and not _already_found(bbox) and qr_seen < 8:
                _decode_picture(image["array"], bbox, location)
                qr_seen += 1
            continue
        chunk = image["array"]
        if bbox is not None and (min(chunk.shape[:2]) < 420 or small_chunk):
            try:
                rendered = render_region(
                    page,
                    bbox,
                    _qr_zoom_for_rect(bbox, target=720 if small_chunk else 420),
                )
            except Exception:
                rendered = None
        ...
        if chunk is None or getattr(chunk, "size", 0) == 0 or _mostly_blank(chunk):
            continue
        if not _already_found(bbox) and qr_seen < 8:
            before = len(placed)
            _decode_picture(chunk, bbox, location)
            if len(placed) == before:
                from vision_scan import _opencv_prepared_hits
                record_hits(
                    _opencv_prepared_hits(chunk),
                    location,
                    bbox if bbox is not None else page_rect,
                    chunk.shape,
                )
            qr_seen += 1
        text_chunk = chunk
        if bbox is not None and _already_found(bbox):
            for item in placed:
                box = item.get("box")
                if box is None:
                    continue
                share = abs(bbox & box) / max(abs(box), 1.0)
                if share < 0.5:
                    continue
                height, width = chunk.shape[:2]
                x0 = int((float(box.x0) - float(bbox.x0)) / max(float(bbox.width), 1.0) * width)
                x1 = int((float(box.x1) - float(bbox.x0)) / max(float(bbox.width), 1.0) * width)
                y0 = int((float(box.y0) - float(bbox.y0)) / max(float(bbox.height), 1.0) * height)
                y1 = int((float(box.y1) - float(bbox.y0)) / max(float(bbox.height), 1.0) * height)
                text_chunk = chunk.copy()
                # Blank only the code so the link printed in this same image is read.
                text_chunk[
                    max(0, y0 - 2) : min(height, y1 + 2),
                    max(0, x0 - 2) : min(width, x1 + 2),
                ] = 255
                break
        if text_chunk is None or getattr(text_chunk, "size", 0) == 0:
            continue
        if _mostly_blank(text_chunk) and not _already_found(bbox):
            continue
        ocr_jobs.append(
            (
                location,
                _source_for_image(bbox, page_rect),
                pool.submit(recognize_text, text_chunk, True),
            )
        )
        logo_seen += 1
    ...
    def _read_small_qr_codes(picture) -> None:
        ...
        for box in boxes[:4]:
            ...
            if len(placed) > before:
                ...
                small = box is not None and max(float(box.width), float(box.height)) < 140
                if small:
                    span = max(int(float(box.width) / max(float(clip.width), 1.0) * array.shape[1]), 8)
                    x0 = int((float(box.x0) - float(clip.x0)) / max(float(clip.width), 1.0) * array.shape[1])
                    _queue_chunk_text(array, (x0, 0, x0 + span, int(array.shape[0])), clip)
    ...
    def _read_bottom_pay_box(picture) -> None:
        """Small pay code at the bottom-left, and the link printed beside it."""
        height, width = int(picture.shape[0]), int(picture.shape[1])
        ...
        crop = picture[y0:, :x1]
        from vision_scan import _opencv_prepared_hits, _qr_points_present
        if crop.size == 0 or not _qr_points_present(crop):
            return
        clip = fitz.Rect(
            page_rect.x0,
            page_rect.y0 + (y0 / height) * float(page_rect.height),
            page_rect.x0 + 0.46 * float(page_rect.width),
            page_rect.y1,
        )
        try:
            sharp = render_region(page, clip, _qr_zoom_for_rect(clip, target=900))
        except Exception:
            sharp = None
        if sharp is None or _mostly_blank(sharp):
            return
        hits = _opencv_prepared_hits(sharp)
        before = len(placed)
        record_hits(hits, "footer", clip, sharp.shape)
        if len(placed) > before and hits:
            _queue_chunk_text(sharp, hits[0].get("box"), clip)

    if full_image is not None:
        _read_small_qr_codes(full_image)
        _read_bottom_pay_box(full_image)
