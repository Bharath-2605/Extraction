...
def extract_from_page(page: fitz.Page, document: fitz.Document) -> tuple[list[dict], str]:
    ...
    for image in images:
        ...
        if image.get("array") is not None and not _already_found(bbox) and (qr_seen < 8 or small_chunk):
            ...
            if not small_chunk:
                qr_seen += 1
            elif _is_qr_like(image["array"], bbox):
                # The printed link sits beside the small code, not inside it.
                beside = fitz.Rect(
                    bbox.x0 - max(70.0, float(bbox.width) * 2.6),
                    bbox.y0 - 6,
                    bbox.x0 - 1,
                    bbox.y1 + 8,
                ) & page_rect
        ...
        if (logo_seen >= 6 and not small_chunk) or frac > 0.18 or image.get("array") is None:
            continue
        chunk = image["array"]
    ...
    def _read_one_pay_box() -> None:
        ...
        for bbox, _array, image in choices[:2]:
            try:
                array = render_region(page, bbox, _qr_zoom_for_rect(bbox, target=900))
            except Exception:
                continue
            if array is None or min(array.shape[:2]) < MIN_IMAGE_SIDE:
                continue
            ...
            if not _already_found(bbox):
                before = len(placed)
                record_hits(decode_qr_hits(array, True), location, bbox, array.shape)
                if len(placed) == before:
                    record_hits(_opencv_prepared_hits(array), location, bbox, array.shape)
                covered.append(bbox)
            if image is not None and image.get("_ocr_done"):
                continue
            ocr_jobs.append((location, "image", pool.submit(recognize_text, array, True)))
