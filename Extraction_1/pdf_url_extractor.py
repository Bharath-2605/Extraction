...
def extract_from_page(page: fitz.Page, document: fitz.Document) -> tuple[list[dict], str]:
    ...
    def _read_one_pay_box() -> None:
        ...
        bbox, array, image = choices[0]
        if image is not None and image.get("_ocr_done") and (image.get("_qr_done") or _already_found(bbox)):
            return
        if array is None:
            try:
                array = render_region(page, bbox, _qr_zoom_for_rect(bbox, target=420))
            except Exception:
                return
        if array is None or min(array.shape[:2]) < MIN_IMAGE_SIDE:
            return
        location = location_for_bbox(tuple(bbox), page_height)
        if not (image is not None and image.get("_qr_done")) and not _already_found(bbox):
            _decode_picture(array, bbox, location)
        if image is not None and image.get("_ocr_done"):
            return
        # The link is printed on the left of this box. The QR sits on the right.
        text_part = array[:, : max(8, int(array.shape[1] * 0.72))]
        ocr_jobs.append((location, "image", pool.submit(recognize_text, text_part, True)))
