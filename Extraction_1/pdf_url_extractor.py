...
def extract_from_page(page: fitz.Page, document: fitz.Document) -> tuple[list[dict], str]:
    ...
    def _ocr_link_beside_qr(box: fitz.Rect | None, clip) -> None:
        ...
        if box is None or max(float(box.width), float(box.height)) > 140:
            return
        ...
        if width > height * 1.35:
            band = fitz.Rect(box.x0 + 2, box.y0 + height * 0.58, box.x1 - width * 0.22, box.y1 + 6)
        else:
            pad = min(200.0, max(width * 4.0, 90.0))
            band = fitz.Rect(box.x0 - pad, box.y0 + height * 0.42, box.x0 - 1, box.y1 + 10)
        band = band & page_rect
        if band.is_empty or float(band.width) < 20 or float(band.height) < 6:
            return
        try:
            picture = render_region(page, band, _qr_zoom_for_rect(band, target=160))
        except Exception:
            return
    ...
    for image in images:
        ...
        if image.get("array") is None or frac > 0.18 or logo_seen >= 6:
            if image.get("array") is not None and not _already_found(bbox) and qr_seen < 8 and small_chunk:
    ...
    def _read_bottom_pay_box(picture) -> None:
        ...
        try:
            sharp = render_region(page, clip, _qr_zoom_for_rect(clip, target=480))
        except Exception:
            sharp = None
