...
def _qr_zoom_for_rect(rect: fitz.Rect, target: float = QR_CROP_MIN_SIDE) -> float:
    ...
    zoom = max(1.8, min(14.0, target / shortest))
    # A wide, short chunk (a pay box) must stay under the QR reader's 1800px limit.
    return min(zoom, max(1.8, 1800.0 / longest))
...
def _keep_complete_url_appearances(rows: list[dict]) -> list[dict]:
    ...
    The same link found again as a hyperlink or by OCR stays one row.
    ...
    allowed = {
        key: {
            "visual": max(bucket["text"], bucket["hyperlink"], bucket["ocr"]),
            "qr": bucket["qr"],
        }
        for key, bucket in counts.items()
    }
    emitted = {key: {"visual": 0, "qr": 0} for key in allowed}
    kept: list[dict] = []
    for row in rows:
        key = str(row.get("url") or "").lower()
        kind = "qr" if row.get("source") == "qr" else "visual"
...
def _one_confusable(left: str, right: str) -> bool:
    ...
    return frozenset(diffs[0]) in {
        frozenset("fl"), frozenset("il"), frozenset("1l"), frozenset("0o"), frozenset("ce"),
    }


def _peel_glued_word(url: str) -> str:
    """securely.virginredrewardscard.syf.com → virginredrewardscard.syf.com"""
    text = str(url or "")
    lowered = text.lower()
    for prefix in ("https://", "http://", "ftp://"):
        if lowered.startswith(prefix):
    ...
    labels = [part for part in host.split(".") if part]
    if len(labels) < 3 or labels[0].lower() not in {"securely", "scan", "paying", "please", "visit"}:
        return url
    rest = ".".join(labels[1:]) + (slash + path if slash else "")
    if not looks_like_url(rest, rest, 0):
        return url
    scheme = ""
    original = str(url)
    for prefix in ("https://", "http://", "ftp://"):
        if original.lower().startswith(prefix):
            scheme = original[: len(prefix)]
            break
    return scheme + rest
...
def _is_pay_box(rect: fitz.Rect | None, page_rect: fitz.Rect) -> bool:
    """The small bordered pay box: a link on the left and a QR on the right."""
    if rect is None or rect.is_empty or rect.is_infinite:
        return False
    width, height = float(rect.width), float(rect.height)
    if width < 48 or height < 20 or width > 360 or height > 180:
        return False
    if width < height * 1.35:
        return False
    page_area = max(float(page_rect.width) * float(page_rect.height), 1.0)
    frac = (width * height) / page_area
    return 0.004 <= frac <= 0.16


def _box_from_border_lines(page: fitz.Page, page_rect: fitz.Rect) -> fitz.Rect | None:
    """The red pay box is four thin strokes, not an image. Join the top and bottom lines."""
    try:
        log = page.get_bboxlog() or []
    except Exception:
        return None
    horizontals: list[fitz.Rect] = []
    for item in log:
        if not item or len(item) < 2 or str(item[0]).lower() != "stroke-path":
            continue
        rect = fitz.Rect(item[1])
        if float(rect.width) >= 80 and float(rect.height) <= 4:
            horizontals.append(rect)
    best: fitz.Rect | None = None
    for index, top in enumerate(horizontals):
        for bottom in horizontals[index + 1 :]:
            upper, lower = (top, bottom) if top.y0 <= bottom.y0 else (bottom, top)
            gap = float(lower.y0 - upper.y1)
            if gap < 28 or gap > 170:
                continue
            overlap = min(float(upper.x1), float(lower.x1)) - max(float(upper.x0), float(lower.x0))
            if overlap < 80:
                continue
            box = fitz.Rect(
                min(float(upper.x0), float(lower.x0)),
                float(upper.y0),
                max(float(upper.x1), float(lower.x1)),
                float(lower.y1),
            )
            if not _is_pay_box(box, page_rect):
                continue
            if best is None or box.x0 < best.x0:
                best = box
    return best


def _drop_cut_off_reads(rows: list[dict]) -> list[dict]:
    """inredrewardscard.syf.com is a crop that cut the front off virginredrewardscard.syf.com."""
    read = [str(row.get("url") or "") for row in rows if row.get("source") in {"image", "logo"}]
    kept: list[dict] = []
    for row in rows:
        url = str(row.get("url") or "")
        if row.get("source") in {"image", "logo"} and url:
            lowered = url.lower()
            if any(
                len(other) > len(url)
                and other.lower().endswith(lowered)
                and other[len(other) - len(url) - 1].isalnum()
                for other in read
            ):
                continue
        kept.append(row)
...
def _text_beside_code(chunk: fitz.Rect, code: fitz.Rect | None, page_rect: fitz.Rect) -> fitz.Rect | None:
    """The part of a wide chunk that is not the code: where the printed link is."""
    if code is None:
        return None
    left = fitz.Rect(chunk.x0, chunk.y0, max(chunk.x0, code.x0 - 1), chunk.y1)
    right = fitz.Rect(min(chunk.x1, code.x1 + 1), chunk.y0, chunk.x1, chunk.y1)
    part = left if float(left.width) >= float(right.width) else right
    part &= page_rect
    if part.is_empty or float(part.width) < 20 or float(part.height) < 6:
        return None
    return part


def _ocr_zoom_for_rect(rect: fitz.Rect) -> float:
    """Large enough for small grey print, without passing the OCR's 1200px limit."""
    width = max(float(rect.width), 1.0)
    height = max(float(rect.height), 1.0)
    return max(2.0, min(1200.0 / width, 360.0 / height, 14.0))


def _boost_contrast(image):
    """Pale grey print on white: stretch it to black so OCR sees each letter."""
    import cv2

    gray = image if getattr(image, "ndim", 0) == 2 else cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    low = float(gray.min())
    high = float(max(gray.max(), low + 1))
    stretched = ((gray.astype("float32") - low) * (255.0 / (high - low))).clip(0, 255).astype("uint8")
    return cv2.cvtColor(stretched, cv2.COLOR_GRAY2RGB)


def _bordered_boxes_in_render(image, page_rect: fitz.Rect) -> list[fitz.Rect]:
    """Red outlined boxes found in the page render, in page points."""
    ...
    red = small[:, :, 0].astype("int16")
    green = small[:, :, 1].astype("int16")
    blue = small[:, :, 2].astype("int16")
    mask = ((red > 140) & (red - green > 60) & (red - blue > 60)).astype("uint8") * 255
    if int(mask.sum()) == 0:
        return []
    mask = cv2.dilate(mask, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)), iterations=1)
    contours, _ = cv2.findContours(mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    sx = float(page_rect.width) / float(small.shape[1])
    sy = float(page_rect.height) / float(small.shape[0])
    boxes: list[fitz.Rect] = []
    for contour in contours:
        x, y, cw, ch = cv2.boundingRect(contour)
        if cw < 20 or ch < 10:
            continue
        # An outline is mostly empty inside. A filled red banner is not a pay box.
        if cv2.contourArea(contour) > 0.6 * cw * ch and float(mask[y : y + ch, x : x + cw].mean()) > 0.45 * 255:
            continue
        rect = fitz.Rect(
            page_rect.x0 + x * sx,
            page_rect.y0 + y * sy,
            page_rect.x0 + (x + cw) * sx,
            page_rect.y0 + (y + ch) * sy,
        )
        if not _is_pay_box(rect, page_rect) or _rect_covered(rect, boxes, overlap=0.6):
            continue
        boxes.append(rect)
...
def extract_from_page(page: fitz.Page, document: fitz.Document) -> tuple[list[dict], str]:
    ...
    def add_ocr(text: str, location: str, source: str) -> None:
        ...
        if not cleaned:
            return
        found = extract_urls_from_any_text(cleaned)
        # Two copies in this same paragraph both stay. A later picture of a link
        # already counted does not add another row.
        seen_here: dict[str, int] = {}
        for url in found:
            url = _peel_glued_word(_snap_spelling(url, page_text))
            key = url.lower()
            seen_here[key] = seen_here.get(key, 0) + 1
            if seen_here[key] == 1 and key in seen_ocr:
                continue
            if seen_here[key] == 1:
                seen_ocr.add(key)
    ...
    def _scan_large_piece(rect: fitz.Rect) -> None:
        ...
        if picture is None or min(picture.shape[:2]) < 24 or _mostly_blank(picture):
            return
        ocr_jobs.append((location_for_bbox(tuple(rect), page_height), "image", pool.submit(recognize_text, picture, True)))
    ...
    for image in images:
        ...
        if hide_qr:
            ...
            continue
        short_side = 0.0 if bbox is None else min(float(bbox.width), float(bbox.height))
        # A small chunk is always opened. The 8-image cap must not skip it.
        small_chunk = 8 <= short_side <= 150
        if image.get("array") is not None and not _already_found(bbox) and (qr_seen < 8 or small_chunk):
            placed_before = len(placed)
            _decode_picture(image["array"], bbox, location)
            image["_qr_done"] = True
            decoded_here = len(placed) > placed_before
            if not small_chunk:
                qr_seen += 1
            elif decoded_here and bbox is not None and not _is_qr_like(image["array"], bbox):
                # A wide chunk holding a code and a printed link: read the text part on its own, larger.
                text_part = _text_beside_code(bbox, placed[-1].get("box"), page_rect)
                if text_part is not None:
                    try:
                        piece = render_region(page, text_part, _ocr_zoom_for_rect(text_part))
                    except Exception:
                        piece = None
                    if piece is not None and min(piece.shape[:2]) >= 8:
                        ocr_jobs.append((location, "image", pool.submit(recognize_text, _boost_contrast(piece), True)))
                        image["_ocr_done"] = True
                        logo_seen += 1
            elif decoded_here and _is_qr_like(image["array"], bbox):
                # The printed link sits beside the small code, not inside it.
                beside = fitz.Rect(
                    bbox.x0 - max(160.0, float(bbox.width) * 3.5),
                    bbox.y0 - 6,
                    bbox.x0 - 1,
                    bbox.y1 + 8,
                ) & page_rect
                if not beside.is_empty and float(beside.width) >= 24:
                    try:
                        band = render_region(page, beside, _qr_zoom_for_rect(beside, target=280))
                    except Exception:
                        band = None
                    if band is not None and min(band.shape[:2]) >= 8:
                        ocr_jobs.append(
                            (
                                location_for_bbox(tuple(beside), page_height),
                                "image",
                                pool.submit(recognize_text, band, True),
                            )
                        )
        # A link printed in a small chunk, logo, or caption under a small code.
        if image.get("_ocr_done"):
            continue
        pay_shaped = _is_pay_box(bbox, page_rect)
        if (logo_seen >= 6 and not pay_shaped) or frac > 0.18 or image.get("array") is None:
            continue
        chunk = image["array"]
    ...
    def _read_one_pay_box() -> None:
        """One small pay-box image. The main list often skips it without a look."""
        choices: list[tuple[fitz.Rect, object, dict | None]] = []
        known: list[fitz.Rect] = []
        for image in images:
            bbox = image.get("bbox")
            if not _is_pay_box(bbox, page_rect):
                continue
            choices.append((bbox, image.get("array"), image))
            known.append(bbox)
        try:
            log = page.get_bboxlog() or []
        except Exception:
        ...
        for item in log:
            ...
            # The red border is a drawn box. The link and QR sit inside it, so it is not listed as an image.
            if "image" not in kind and kind not in {"stroke-path", "fill-path"}:
                continue
            rect = fitz.Rect(item[1]) if len(item) > 1 else None
            if not _is_pay_box(rect, page_rect) or _rect_covered(rect, known, overlap=0.72):
                continue
            choices.append((rect, None, None))
            known.append(rect)
        border = _box_from_border_lines(page, page_rect)
        if border is not None and not _rect_covered(border, known, overlap=0.72):
            choices.append((border, None, None))
            known.append(border)
        # A scanned page is one big picture, so the box is only visible in the page render.
        for rect in _bordered_boxes_in_render(full_image, page_rect):
            if not _rect_covered(rect, known, overlap=0.6):
                choices.append((rect, None, None))
                known.append(rect)
        if not choices:
            return
        choices.sort(key=lambda item: (0 if item[0].x0 < page_rect.width * 0.55 else 1, item[0].y0))
        from vision_scan import _opencv_prepared_hits

        handled = [
            item["bbox"]
            for item in images
            if item.get("bbox") is not None and item.get("_ocr_done") and item.get("_qr_done")
        ]
        for bbox, _array, image in choices[:2]:
            if _rect_covered(bbox, handled, overlap=0.6):
                continue
            location = location_for_bbox(tuple(bbox), page_height)
            if not _already_found(bbox):
                try:
                    array = render_region(page, bbox, _qr_zoom_for_rect(bbox, target=480))
                except Exception:
                    array = None
                if array is not None and min(array.shape[:2]) >= MIN_IMAGE_SIDE:
                    before = len(placed)
                    record_hits(decode_qr_hits(array, False), location, bbox, array.shape)
                    if len(placed) == before:
                        record_hits(_opencv_prepared_hits(array), location, bbox, array.shape)
                    covered.append(bbox)
            if image is not None and image.get("_ocr_done"):
                continue
            code_box = next(
                (item.get("box") for item in placed if item.get("box") is not None and _rect_covered(item["box"], [bbox], overlap=0.6)),
                None,
            )
            text_part = _text_beside_code(bbox, code_box, page_rect) if code_box is not None else bbox
