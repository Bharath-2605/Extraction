...
def extract_from_page(page: fitz.Page, document: fitz.Document) -> tuple[list[dict], str]:
    ...
    for image in images:
        ...
        if hide_qr:
            ...
            continue
        small = (
            image.get("array") is not None
            and frac <= 0.18
            and logo_seen < 6
        )
        if image.get("array") is not None and not small and not _already_found(bbox) and qr_seen < 8:
            _decode_picture(image["array"], bbox, location)
            qr_seen += 1
        # A link printed in a small chunk, logo, or caption next to a small code.
        if not small:
            continue
        chunk = image["array"]
        ocr_box = bbox
        if bbox is not None and _is_qr_like(chunk, bbox):
            # The printed link sits to the left of this small code, in the same box.
            pad = min(240.0, max(float(bbox.width) * 3.6, 90.0))
            ocr_box = fitz.Rect(bbox.x0 - pad, bbox.y0 - 4, bbox.x1 + 4, bbox.y1 + 6) & page_rect
        elif bbox is not None and not _already_found(bbox) and qr_seen < 8:
            # A wide payment box has the code on the right. Straighten that square.
            ...
            if len(placed) > before:
                qr_seen += 1
        if ocr_box is not None and (
            min(chunk.shape[:2]) < 420 or (bbox is not None and ocr_box.width > bbox.width + 8)
        ):
            try:
                rendered = render_region(page, ocr_box, _qr_zoom_for_rect(ocr_box, target=420))
            except Exception:
                rendered = None
            if rendered is not None and min(rendered.shape[:2]) >= MIN_IMAGE_SIDE:
                if not _is_qr_like(chunk, bbox) and min(rendered.shape[:2]) > min(chunk.shape[:2]):
                    chunk = rendered
                elif _is_qr_like(chunk, bbox):
                    chunk = rendered
    ...
    def _read_pay_box() -> None:
        """A 'scan and pay' line with the code to its right, even when it is not its own image."""
        hits = []
        for span in spans:
            text = str(span.get("text") or "")
            lowered = text.lower()
            if "scan and pay" in lowered or "paying your bill" in lowered or "scan & pay" in lowered:
                box = span.get("bbox")
                if box:
                    hits.append(fitz.Rect(box))
        if not hits:
            return
        union = hits[0]
        for box in hits[1:]:
            union |= box
        clip = fitz.Rect(union.x0 - 8, union.y0 - 8, union.x1 + 90, union.y1 + 28) & page_rect
        if clip.is_empty or min(float(clip.width), float(clip.height)) < 12:
            return
        try:
            picture = render_region(page, clip, _qr_zoom_for_rect(clip, target=520))
        except Exception:
            return
        if picture is None or _mostly_blank(picture):
            return
        from vision_scan import _opencv_prepared_hits
        before = len(placed)
        record_hits(
            decode_qr_hits(picture, False) or _opencv_prepared_hits(picture),
            location_for_bbox(tuple(clip), page_height),
            clip,
            picture.shape,
        )
        if len(placed) == before and _already_found(clip):
            pass
        ocr_jobs.append(
            (
                location_for_bbox(tuple(clip), page_height),
    ...
    def _read_small_qr_codes(picture) -> None:
        ...
        try:
            ...
            boxes = []
        height = int(picture.shape[0]) or 1

        def _lower_first(box) -> tuple:
            _x0, y0, _x1, y1 = box
            return (0 if ((y0 + y1) / 2) >= height * 0.55 else 1, (y1 - y0) * (_x1 - _x0))

        for box in sorted(boxes, key=_lower_first)[:6]:
            ...
            from vision_scan import _opencv_prepared_hits
            record_hits(
                decode_qr_hits(array, False) or _opencv_prepared_hits(array),
                location_for_bbox(tuple(clip), page_height),
                clip,
                array.shape,
            )
            if len(placed) > before and clip.width < 160 and clip.height < 160:
                pad = min(240.0, max(float(clip.width) * 3.6, 90.0))
                band = fitz.Rect(clip.x0 - pad, clip.y0 - 4, clip.x1 + 4, clip.y1 + 6) & page_rect
                try:
                    beside = render_region(page, band, _qr_zoom_for_rect(band, target=420))
                except Exception:
                    beside = None
                if beside is not None and not _mostly_blank(beside):
                    ocr_jobs.append(
                        (
                            location_for_bbox(tuple(band), page_height),
                            "image",
                            pool.submit(recognize_text, beside, True),
                        )
                    )
