...
def extract_from_page(page: fitz.Page, document: fitz.Document) -> tuple[list[dict], str]:
    ...
    for image in images:
        ...
        if bbox is not None and min(chunk.shape[:2]) < 420:
            ...
            if rendered is not None and min(rendered.shape[:2]) > min(chunk.shape[:2]):
                chunk = rendered
        if _already_found(bbox) and _is_qr_like(chunk, bbox) and bbox is not None:
            pad = min(240.0, max(float(bbox.width) * 3.6, 90.0))
            band = fitz.Rect(bbox.x0 - pad, bbox.y0 - 2, bbox.x0 + 2, bbox.y1 + 10) & page_rect
            try:
    ...
    def _read_bottom_pay_box(picture) -> None:
        """The pay box under the promo: small code on the left, link beside it."""
        height, width = int(picture.shape[0]), int(picture.shape[1])
        ...
        try:
            sharp = render_region(page, clip, _qr_zoom_for_rect(clip, target=640))
        except Exception:
            sharp = None
        ...
        text_image = sharp
        if hits and hits[0].get("box"):
            x0, y0, x1, y1 = hits[0]["box"]
            height, width = sharp.shape[:2]
            left = sharp[max(0, int(y0) - 8):min(height, int(y1) + 12), :max(8, int(x0) - 2)]
            if left.size and min(left.shape[:2]) >= 8:
                text_image = left
        ocr_jobs.append(("footer", "image", pool.submit(recognize_text, text_image, True)))
