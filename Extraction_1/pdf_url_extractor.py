    ...
    for image in images:
        ...
        if bbox is not None and _already_found(bbox):
            for item in placed:
                ...
                if share < 0.5:
                    continue
                cut = int((float(box.x0) - float(bbox.x0)) / max(float(bbox.width), 1.0) * chunk.shape[1])
                if 8 < cut < chunk.shape[1] - 8:
                    text_chunk = chunk[:, :cut]
                break
        if text_chunk is None or getattr(text_chunk, "size", 0) == 0 or _mostly_blank(text_chunk):
            continue
        ocr_jobs.append(
            (
                location,
