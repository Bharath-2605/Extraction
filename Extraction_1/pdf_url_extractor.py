...
def extract_from_page(page: fitz.Page, document: fitz.Document) -> tuple[list[dict], str]:
    ...
    def _ocr_link_beside_qr(box: fitz.Rect | None, clip) -> None:
        """Read the link printed to the left of a small code."""
        if box is None or max(float(box.width), float(box.height)) > 110:
            return
        pad = min(260.0, max(float(box.width) * 5.0, 140.0))
        band = fitz.Rect(box.x0 - pad, box.y0 - 8, box.x0 - 1, box.y1 + 22) & page_rect
        if band.is_empty or float(band.width) < 24:
            return
        try:
            picture = render_region(page, band, _qr_zoom_for_rect(band, target=420))
        except Exception:
            return
        if picture is None or min(picture.shape[:2]) < 8:
            return
        sample = picture if getattr(picture, "ndim", 0) == 2 else picture.min(axis=2)
        if float((sample < 90).mean()) < 0.001:
            return
        ocr_jobs.append(
            (
