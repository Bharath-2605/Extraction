...
def _drop_embedded_host_urls(rows: list[dict], text: str) -> list[dict]:
    ...
    # A QR payload is not printed text. The link printed beside the code is its own appearance.
    found = [str(row.get("url") or "") for row in rows if row.get("source") != "qr"]
    used: dict[str, int] = {}
    kept: list[dict] = []
    for row in rows:
...
def _peel_glued_word(url: str) -> str:
    ...
    for prefix in ("https://", "http://", "ftp://"):
        if original.lower().startswith(prefix):
            scheme, text = original[: len(prefix)], original[len(prefix):]
            break
    host, slash, path = text.partition("/")
    labels = [part for part in host.split(".") if part]
    if len(labels) < 3 or labels[0].lower() not in {"securely", "scan", "paying", "please", "visit", "call"}:
        return original
    rest = ".".join(labels[1:]) + (slash + path if slash else "")
    return scheme + rest if looks_like_url(rest, rest, 0) else original
...
def _is_pay_box(rect: fitz.Rect | None, page_rect: fitz.Rect) -> bool:
    """The small bordered pay box: a link on one side and a QR on the other."""
    if rect is None or rect.is_empty or rect.is_infinite:
        return False
    width, height = float(rect.width), float(rect.height)
    if width < 48 or height < 20 or width > 360 or height > 180 or width < height * 1.35:
        return False
    page_area = max(float(page_rect.width) * float(page_rect.height), 1.0)
    return 0.004 <= (width * height) / page_area <= 0.16


def _pay_boxes(page: fitz.Page, page_rect: fitz.Rect, render) -> list[fitz.Rect]:
    """Pay boxes stored as a picture, as drawn border lines, or only visible in the page render."""
    ...
    for item in log:
        ...
        if "image" in kind or kind in {"stroke-path", "fill-path"}:
            if _is_pay_box(rect, page_rect) and not _rect_covered(rect, boxes, overlap=0.6):
                boxes.append(rect)
        if kind == "stroke-path" and float(rect.width) >= 80 and float(rect.height) <= 4:
            lines.append(rect)
    for index, top in enumerate(lines):
        for bottom in lines[index + 1 :]:
            upper, lower = (top, bottom) if top.y0 <= bottom.y0 else (bottom, top)
            if not 28 <= float(lower.y0 - upper.y1) <= 170:
                continue
            box = fitz.Rect(min(upper.x0, lower.x0), upper.y0, max(upper.x1, lower.x1), lower.y1)
            if _is_pay_box(box, page_rect) and not _rect_covered(box, boxes, overlap=0.6):
                boxes.append(box)
    for rect in _red_boxes_in_render(render, page_rect):
        if not _rect_covered(rect, boxes, overlap=0.6):
            boxes.append(rect)
    return boxes[:3]


def _red_boxes_in_render(image, page_rect: fitz.Rect) -> list[fitz.Rect]:
    """Red outlined boxes in the page render. A scanned page has no separate pictures."""
    if image is None or getattr(image, "ndim", 0) != 3 or image.shape[2] < 3:
        return []
    import cv2

    scale = min(1.0, 1100.0 / max(image.shape[1], 1))
    ...
    for contour in contours:
        ...
        if float(mask[y : y + ch, x : x + cw].mean()) > 0.45 * 255:
            continue
        rect = fitz.Rect(
            page_rect.x0 + x * sx, page_rect.y0 + y * sy,
            page_rect.x0 + (x + cw) * sx, page_rect.y0 + (y + ch) * sy,
        )
        if _is_pay_box(rect, page_rect) and not _rect_covered(rect, boxes, overlap=0.6):
            boxes.append(rect)
    return boxes[:2]
...
def _link_area_near_code(code: fitz.Rect, page_rect: fitz.Rect) -> fitz.Rect | None:
    """Where the link is printed for a small code: the line of print beside it and just under it."""
    width, height = float(code.width), float(code.height)
    if min(width, height) < 8:
        return None
    # The detector sometimes returns the whole chunk, not the small square.
    # The printed link is the left part of that chunk. A tight code still uses the band beside it.
    if width > height * 1.35 and width > 150:
        area = fitz.Rect(code.x0, code.y0, code.x0 + width * 0.72, code.y1) & page_rect
        if area.is_empty or float(area.width) < 24:
            return None
        return area
    if min(width, height) > 150:
        return None
    area = fitz.Rect(
        code.x0 - max(170.0, width * 4.0),
        code.y0 - height * 0.25,
        code.x1 + width * 0.3,
        code.y1 + height * 0.4,
    ) & page_rect
    if area.is_empty or float(area.width) < 24 or float(area.height) < 8:
        return None
    return area


def _ocr_zoom_for_rect(rect: fitz.Rect) -> float:
    """Large enough for small grey print, within the OCR's 1200px limit."""
    return max(2.0, min(1400.0 / max(float(rect.width), 1.0), 480.0 / max(float(rect.height), 1.0), 16.0))


_OCR_TLD_FIXES = (
    (re.compile(r"\.\s*(?:c0m|corn|conn|cam|co m|c om)\b", re.IGNORECASE), ".com"),
    (re.compile(r"\.\s*(?:0rg|orq)\b", re.IGNORECASE), ".org"),
    (re.compile(r"\.\s*(?:nct|n et)\b", re.IGNORECASE), ".net"),
)


def _fix_ocr_tld(text: str) -> str:
    """Small print often comes back as .corn or .c0m. The rest of the link is left as read."""
    fixed = str(text or "")
    for pattern, replacement in _OCR_TLD_FIXES:
...
def _drop_cut_off_reads(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        url = str(row.get("url") or "")
        if row.get("source") in {"image", "logo"} and url and any(
            len(other) > len(url)
            and other.lower().endswith(url.lower())
...
def extract_from_page(page: fitz.Page, document: fitz.Document) -> tuple[list[dict], str]:
    ...
    for image in images:
        ...
        if logo_seen >= 6 or frac > 0.18 or image.get("array") is None:
            continue
        chunk = image["array"]
        # A very small picture: read its own pixels. Stretching it on the page only blurs the letters.
        if (
            chunk is not None
            and chunk.shape[0] < 64
            and chunk.shape[1] > chunk.shape[0] * 1.4
        ):
            from vision_scan import recognize_lines

            text_side = chunk[:, : max(8, int(chunk.shape[1] * 0.75))]
            found_link = False
            for line in recognize_lines(text_side).splitlines():
                line = _fix_ocr_tld(line)
                found_link = found_link or bool(extract_urls_from_any_text(line))
                add_ocr(line, location, "image")
            if found_link:
    ...
    # A pay box whose code nothing above decoded yet.
    for box in _pay_boxes(page, page_rect, full_image):
        if any(item.get("box") is not None and _rect_covered(item["box"], [box], overlap=0.6) for item in placed):
            continue
        try:
            array = render_region(page, box, _qr_zoom_for_rect(box, target=480))
        except Exception:
            continue
        if array is not None and min(array.shape[:2]) >= MIN_IMAGE_SIDE:
            _decode_picture(array, box, location_for_bbox(tuple(box), page_height))
        text_box = fitz.Rect(box.x0, box.y0, box.x0 + float(box.width) * 0.72, box.y1)
        try:
            from vision_scan import recognize_lines

            piece = render_region(page, text_box, _ocr_zoom_for_rect(text_box))
        except Exception:
            piece = None
        if piece is not None and min(piece.shape[:2]) >= 8:
            where = location_for_bbox(tuple(text_box), page_height)
            for line in recognize_lines(piece).splitlines():
                add_ocr(_fix_ocr_tld(line), where, "image")

    # RapidOCR, line by line, on the print beside each small code: the link next to a QR chunk.
    from vision_scan import recognize_lines

    near_read = 0
    for item in placed:
        box = item.get("box")
        area = _link_area_near_code(box, page_rect) if box is not None else None
        if area is None or near_read >= 3:
            continue
        try:
            piece = render_region(page, area, _ocr_zoom_for_rect(area))
        except Exception:
            continue
        if piece is None or min(piece.shape[:2]) < 8:
            continue
        # The code itself is not print. Blank it so it is not read as a line.
        scale_x = piece.shape[1] / max(float(area.width), 1.0)
        scale_y = piece.shape[0] / max(float(area.height), 1.0)
        cx0 = max(0, int((box.x0 - area.x0) * scale_x))
        cy0 = max(0, int((box.y0 - area.y0) * scale_y))
        cx1 = min(piece.shape[1], int((box.x1 - area.x0) * scale_x) + 1)
        cy1 = min(piece.shape[0], int((box.y1 - area.y0) * scale_y) + 1)
        piece = piece.copy()
        piece[cy0:cy1, cx0:cx1] = 255
        where = location_for_bbox(tuple(area), page_height)
        found_link = False
        # One line at a time, so "No need to call." is not joined to the next line as call.com.
        for line in recognize_lines(piece).splitlines():
            line = _fix_ocr_tld(line)
            found_link = found_link or bool(extract_urls_from_any_text(line))
            add_ocr(line, where, "image")
        if not found_link:
            # The line cut missed it: one full pass with lower cut-offs for faint print.
            from vision_scan import recognize_faint_text

            for line in recognize_faint_text(piece).splitlines():
                add_ocr(_fix_ocr_tld(line), where, "image")
        near_read += 1
