...
def recognize_lines(image: np.ndarray) -> str:
    ...
    ink = (darkest < paper - 12) & ~((red - green > 45) & (red - blue > 45))
    ...
    for y0, y1 in bands[:6]:
        ...
        gray = _to_gray(line)
        height = gray.shape[0]
        # Grow only a line that is still a few pixels tall. A line that is already
        # large is left alone, because stretching it makes the letters worse.
        if 0 < height < 28:
            scale = 36.0 / float(height)
            gray = cv2.resize(gray, None, fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)
        if gray.shape[1] > 2000:
            scale = 2000.0 / float(gray.shape[1])
