...
def _rapidocr_texts(engine, image: np.ndarray) -> list[str]:
...
def _detector_qr_hits(image: np.ndarray) -> list[dict]:
    ...
    if max(height, width) <= 1000:
        return _redrawn_qr_hits(gray, rgb)
    return hits
...
def _scan_strip_windows(image: np.ndarray, found: list[dict], scan) -> None:
    ...
    step = max(1, int(side * 0.5))
    start = max(0, length - side)
    # The code in a pay box sits at the right end, so look there first.
    positions = list(range(start, -1, -step))
    if positions[-1] != 0:
        positions.append(0)
